...
export const WORKFLOW_MANIFEST: Record<CanonicalGjcWorkflowSkill, SkillManifest> = {
	...
	autoresearch: manifest({
		...
		verbs: [
			verb("handoff", "state-action"),
			...flagVerbs(["spec"]),
			...positionalVerbs(["read", "clear", "write", "log-run", "critic", "verdict", "report"]),
			...plannedVerbs(PLANNED_ADMIN_VERBS),
		],
		typedArgs: [
			{ name: "spec", type: "string", appliesToVerbs: ["spec"] },
			...
			{ name: "slug", type: "string", appliesToVerbs: ["write", "log-run", "critic", "verdict"] },
			{ name: "run-id", type: "string", required: true, appliesToVerbs: ["log-run"] },
			{ name: "status", type: "string", required: true, appliesToVerbs: ["log-run"] },
			{ name: "description", type: "string", required: true, appliesToVerbs: ["log-run"] },
			{ name: "status-json", type: "string", required: true, appliesToVerbs: ["critic", "verdict"] },
			{ name: "evidence", type: "string", appliesToVerbs: ["critic", "verdict"] },
			{ name: "evaluator", type: "string", required: true, appliesToVerbs: ["critic", "verdict"] },
			{
				name: "json",
				type: "boolean",
				appliesToVerbs: ["spec", "read", "write", "clear", "log-run", "critic", "verdict", "report"],
			},
			{ name: "args", type: "string", planned: true },
			{ name: "metadata-json", type: "string", planned: true },
		],
